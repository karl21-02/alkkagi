...
import os
# Disable audio before importing pygame (prevents ALSA errors)
...
# Note: 'reduce-overhead' mode uses CUDAGraphs which can conflict with inference_mode()
# Using 'default' mode for better compatibility
USE_COMPILE = False  # Disabled due to CUDAGraphs compatibility issues
...
SAVE_PATH = "my_alkkagi_agent_new_ppo.pkl"
...
class BaseAgent(kym.Agent):
    def __init__(self, my_turn: int):
        ...
        # Pre-allocated tensors for faster inference
    ...
    def get_action(self, obs_np: np.ndarray, stone_mask: np.ndarray,
        ...
        # Reuse pre-allocated tensors or create new ones
        if self._obs_buffer is None or self._obs_buffer.shape[0] != batch_size:
            self._obs_buffer = torch.empty((batch_size, self.state_dim), dtype=torch.float32, device=DEVICE)
            self._mask_buffer = torch.empty((batch_size, 3), dtype=torch.float32, device=DEVICE)

        # Copy data to pre-allocated tensors (faster than creating new tensors)
...
def train():
    ...
    # Define and register signal handler only in train()
    ...
    print("-" * 70)
    print(f"{'Upd':>6} | {'Score':>8} | {'Win%':>6} | {'ELO':>6} | {'K/D':>9} | {'Opp':>15} | {'Time':>8}")
    print("-" * 70)
    ...
    for update in range(start_update, MAX_UPDATES + 1):
        ...
        for _ in range(T_HORIZON):
            ...
            if len(my_idx) > 0:
                ...
                # Track kills/deaths (only count positive changes, ignore resets)
                for i, env_i in enumerate(my_idx):
                    if not (term[env_i] or trunc[env_i]):  # Skip reset frames
        ...
        if update % 10 == 0:
            ...
            # Print formatted log
            print(f"{update:6d} | {avg_score:8.2f} | {recent_win_rate:5.1f}% | {op_manager.current_elo:6.0f} | "
                  f"{total_kills:5d}/{total_deaths:<5d} | {opponent_name:>15} | {elapsed_str}")

            agent.save(SAVE_PATH, update)

            # Periodic detailed stats (every 100 updates)
            if update % 100 == 0:
                print("-" * 70)
    ...
    agent.save(SAVE_PATH, update)
    op_manager.elo.save(os.path.join(HISTORY_DIR, ELO_FILE))

    total_time = time.time() - start_time
    print(f"  Total Time    : {str(timedelta(seconds=int(total_time)))}")
    print(f"  Final ELO     : {op_manager.current_elo:.0f}")
